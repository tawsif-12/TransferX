...
const noHtmlString = (minLength = 1) =>
  z
    .string()
    .min(minLength)
    .refine((v) => !/[<>]/.test(v), {
...
export const signupSchema = z.object({
  email: z.string().email('Invalid email format').refine((v) => !/[<>]/.test(v), {
    message: 'Email cannot contain HTML characters',
  }),
  password: z.string().min(6, 'Password must be at least 6 characters'),
  fullName: noHtmlString(2),
